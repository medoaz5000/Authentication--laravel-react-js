...
export default function Home() {
    ...
    const handleLogout = () => {
        ...
        .catch(() => {
            // even if request fails → force logout
            setUser(null);
            setToken(null);
  ...
  return (
    <>
            ...
            <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                    ...
                    <div className="flex space-x-4">
                        ...
                        {!token ? (
                            <>
                                <Link to="/login" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Log in</Link>
                                <Link to="/register" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Sign up</Link>
                            </>
                        ) : (
                            <>
                                <Link to="/" aria-current="page" className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white">Dashboard</Link>
                                <button className="rounded-md hover:bg-white/5 px-3 py-2 text-sm text-gray-300 font-medium hover:text-white" onClick={handleLogout}>
                                Log out
                                </button>
                            </>
                        ...
                        <Link to="/login" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Log in</Link>
                        <Link to="/register" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Sign up</Link>
                        <button className="rounded-md hover:bg-white/5 px-3 py-2  text-sm text-gray-300 font-medium hover:text-white" onClick={handleLogout} >Log out</button>   */}                 </div>
                    </div>
        ...
        <div>
            <span>
                <p>{user?.name ? `Welcome admin, ${user.name}` : "Guest"} </p> 
            </span>
        </div>
    </>
  )
